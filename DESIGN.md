---
name: ADP Viewer
description: Literary-journal reading surface for account development plans — true paper, deep indigo, set type.
colors:
  paper: "#F8F7F4"
  surface: "#FFFFFF"
  surface-sunken: "#F0EFEB"
  ink: "#1A1817"
  ink-muted: "#3A3735"
  ink-subtle: "#56524E"
  ink-faint: "#75716C"
...
The system carries one accent — **deep indigo**, the ink-like hue of a scholarly paperback — across a true off-white paper. Warmth lives in the ink and the indigo, never in a cream-tinted background. Type is the hero: Fraunces for display, Newsreader for the body, JetBrains Mono for code. Everything else recedes so the reading can come forward.
...
- One accent, used rarely: indigo marks links, active state, and primary callouts only.
...
## 2. Colors: The Indigo-on-Paper Palette

One hue family carries the system. Indigo (~275° in OKLCH) threads through the paper's faint tint, the primary accent, the active-state wash, and the dark theme's surfaces. Neutrals are near-true off-white, not cream.

### Primary
- **Deep Indigo** (`#2A2E5A`, oklch 30% 0.08 275): the single brand accent. Used on links, active tree item text, focus rings, primary callout (note), and the theme-toggle hover border. Never used as a fill on large surfaces; its rarity is the point. On paper it hits ~10:1 contrast; on dark ink it lightens to **Periwinkle Indigo** (`#8B8FD6`, oklch 62% 0.10 275) to stay legible against near-black.
...
- **Oxblood Red** (`#7A2018` on `#EDD5CE`): stale — the alert. Deliberately not pure red; oxblood reads as serious, not panicky. Keep clear of indigo so it never blurs into the primary.

### Tertiary (callout semantics)
- **Indigo Note** — primary callout, shares the brand accent.
- **Sage Tip**, **Amber Warn**, **Violet Design**, **Neutral Aside** — see Components § Callouts.

### Neutral
- **Paper** (`#F8F7F4`, oklch 97% 0.004 275): the body background. True off-white with a hair of indigo tint — NOT cream. The previous cream bg (`#FBF7F0`) sat in the saturated AI-default warm-neutral band; this escapes it while still reading as paper.
...
- **Border** (`#E1E0DC`), **Border Strong** (`#CAC9C5`): neutral hairlines, cool-neutral to match the indigo thread.

### Named Rules
**The One Voice Rule.** Indigo is the only accent. Status colors (gray/amber/sage/oxblood) are semantic signals on badges and callout borders — never decorative, never on body text, never used as a second brand color. If a screen has two accents fighting, one is wrong.

**The No-Cream Rule.** The body background is true off-white at near-zero chroma, tinted toward indigo — never toward warmth by default. Warmth enters through ink, accent, and typography. A cream/sand/paper token name is a tell that the palette has drifted back to the AI default.

**The Dark-Mirror Rule.** Dark theme is the light theme mirrored through ink, not a separate "tool mode." Same hue thread (indigo 275°), same semantic roles, same type. The dark accent is periwinkle indigo (`#8B8FD6`), deliberately kept deeper than tech-blue range.
...
### Shadow Vocabulary
- **Ambient Low** (`box-shadow: 0 1px 2px rgba(25, 25, 40, .08)`): the resting lift on cards. Barely visible; its job is to separate the card from paper, not to float it.
- **Ambient Mid** (`box-shadow: 0 4px 14px rgba(25, 25, 40, .13)`): reserved for future elevated surfaces (dropdowns, dialogs). Currently unused in the resting UI.
- **Overflow Cue** (`box-shadow: inset -18px 0 14px -12px rgba(25, 25, 40, .18)`): an inset fade on the right edge of a `<pre>` that overflows its column. Signals "scroll right," nothing else.
- **Dark mirrors** (`rgba(0, 0, 0, .30)` / `.45`): the dark-theme counterparts, stronger because the surfaces are darker.

Shadow rgba values are cool-neutral (`25, 25, 40`) to match the indigo thread, never warm (`40, 30, 20`) — the warm shadow was a leftover from the previous ember palette.
...
- **Handoff button (`.btn`):** surface background, ink text, 1px border, Fraunces 12px label, `4px 10px` padding. Hover → surface-sunken bg + indigo border. Active → 1px downward nudge (`translateY(1px)`). This is the only button style; there is no primary-fill button.
- **Code copy (`.code-copy`):** surface bg, ink-subtle text, Fraunces 600 0.72rem uppercase 0.04em tracking. Hidden by default, revealed on `.code-block:hover`. Copied state → indigo text. A quiet, expert affordance.
- **Theme toggle (`.theme-toggle`):** surface bg, icon + Fraunces label, 4px radius. Hover → indigo border. Carries sun/moon glyphs (☀ / ☾), not text icons.

### Links
- Indigo, underlined at 45% opacity, 1px thickness, 2px offset. Hover → indigo-deep, full-opacity underline. Never bold, never a button-styled link.
...
- **Focus:** indigo border + 3px indigo-tinted ring (`color-mix(in srgb, indigo 20%, transparent)`). The focus state is the only place indigo appears as a fill-ish element.
...
- **File tree (sidebar):** surface-sunken bg, 280px fixed width. Items: 13px Newsreader, folder/file glyphs (📁/📄 — flagged for replacement in `/impeccable craft`), active item = indigo-tint bg + indigo text + 500 weight. Hover = border-muted bg.
...
- **Five semantics:** note (indigo — primary), tip (sage), warning/headsup (amber), designnote (violet), aside (transparent bg, italic body, neutral border).
...
### Do:
- **Do** use indigo (`#2A2E5A`) as the one and only accent — on links, active tree state, focus rings, and the note callout. Its rarity is the system's voice.
- **Do** keep body paragraph text at `--ink-muted` (`#3A3735`) or darker. Reading text below ~7:1 on paper fails the brief.
- **Do** thread the indigo hue (~275°) through every neutral — paper tint, borders, shadows, dark surfaces. Coherence is the tell that the palette was designed, not assembled.
...
- **Do** run all type and color decisions through the dark mirror: if it doesn't hold at `#111013` paper with `#8B8FD6` indigo, it's wrong in light too.

### Don't:
- **Don't** use a cream / sand / parchment / paper-named body background. PRODUCT.md flags the warm-neutral cream band as the saturated AI default of 2026; the previous `#FBF7F0` was squarely in it. Body bg stays true off-white at near-zero chroma, tinted toward indigo.
...
- **Don't** apply gradient text (`background-clip: text` + gradient). Emphasis comes from weight, size, or the single indigo accent — never from a gradient.
